# Privacy Policy for RoadTrip

**Last Updated:** August 22, 2026

RoadTrip is a free app that lets a group of people share their live location with each other on the way to a shared destination. This page explains what data RoadTrip collects, how it's used, and how it's shared, so you can decide whether to use the app.

RoadTrip has **no accounts and no login screen**. There's no email, password, or phone number involved anywhere in the app.

## Information We Collect

### Display Name
The first time you open RoadTrip, you're asked for a display name — just so the rest of your group can tell you apart on the map. This name is stored only on your device. It's shared with the other members of a journey **only while you're actively part of that journey**, so they can see whose location marker is whose.

### Location Data
While you're actively part of a journey (from the moment you create or join one, until you leave or it ends), RoadTrip reads your device's GPS location and shares it with the other members of that same journey so everyone can see each other's position and distance to the destination on a live map.
...
- When you start or join a journey, you're asked whether to allow "Always" (background) location access so your position keeps updating for the group even while you're using another app, like Maps for directions. This is optional: if you decline or only grant "While Using the App," RoadTrip falls back to foreground-only tracking with no loss of functionality besides that pause when backgrounded.
- Whenever background tracking is active, RoadTrip shows a persistent notification ("Journey in progress") for as long as it's happening, so it's never hidden from you.
...
### Device Identifier
RoadTrip uses Firebase Anonymous Authentication to give your device a random, non-identifying ID. This ID isn't linked to your name, email, or any other personal information — it exists purely so the app's backend can tell which location update came from which device within a journey, and to enforce that you can only ever update your own location, not anyone else's.

### Camera
If you scan a QR code to join a journey, RoadTrip briefly uses your camera to read the code. Nothing captured by the camera is stored, saved, or transmitted anywhere — it's used only to decode the QR code on your device in that moment.
...
RoadTrip relies on the following third-party services to function. Each is governed by its own privacy policy, which we encourage you to review:
